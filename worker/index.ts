...
async function licitacoesApi(request: Request, env: Env, url: URL) {
  if (request.method === "GET") {
    await env.DB.batch([
      ...
      env.DB.prepare("UPDATE licitacao_items SET valor_vendido = 0 WHERE licitacao_id IN (SELECT id FROM licitacoes WHERE status = 'Proposta Enviada')"),
    ]);
  ...
  if (request.method === "PATCH" && updateItemMatch) {
    ...
    await env.DB.prepare("UPDATE licitacao_items SET status_compra=COALESCE(?,status_compra), valor_vendido=CASE WHEN ? IS NULL THEN valor_vendido WHEN ? <= 0 THEN 0 ELSE MAX(?,valor_minimo) END WHERE id=? AND licitacao_id=?")
      .bind(body.status_compra ?? null, sale, sale, sale, Number(updateItemMatch[2]), Number(updateItemMatch[1])).run();
    return Response.json({ ok: true });
  }
